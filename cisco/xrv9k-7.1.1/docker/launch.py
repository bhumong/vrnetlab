...
class XRv9k_vm(vrnetlab.VM):
    def __init__(
        ...
        # Check if this is a VGA image that requires special handling
        self.is_vga_image = "vga" in self.image.lower()

        # Track VGA console authentication attempts
        self.vga_auth_attempts = 0
        self.max_vga_auth_attempts = 3
        self.vga_auth_delay = 10  # seconds between attempts
        ...
        # For VGA images, we need to enable VNC for console access
        # Replace -display none with VNC for VGA images
        if self.is_vga_image:
            for i in range(len(self.qemu_args)):
                if self.qemu_args[i] == "-display" and i + 1 < len(self.qemu_args):
                    if self.qemu_args[i + 1] == "none":
                        self.qemu_args[i + 1] = "vnc=0.0.0.0:0"
                        self.logger.info(
                            "VGA image detected: enabling VNC console on port 5900"
                        )
                        self.logger.info(
                            "Connect via VNC to complete initial setup for VGA image"
                        )
                    break
    ...
    def bootstrap_spin(self):
        ...
        (ridx, match, res) = self.con_expect(
            [
                ...
                b"Username:",  # For VGA console login
                b"Password:",  # For VGA console login
            ],
        )
        ...
        if match:  # got a match!
            ...
            if ridx == 2 and self.install_mode:
                ...
                self.running = True
                return
            if ridx == 3 and self.is_vga_image:  # Username prompt on VGA console
                if self.vga_auth_attempts < self.max_vga_auth_attempts:
                    self.logger.info(
                        f"Caught VGA console username prompt (attempt {self.vga_auth_attempts + 1}/{self.max_vga_auth_attempts})"
                    )
                    self.wait_write(self.username, wait=None)
                else:
                    self.logger.info(
                        "Max VGA auth attempts reached, letting manual VNC session handle authentication"
                    )
            if ridx == 4 and self.is_vga_image:  # Password prompt on VGA console
                if self.vga_auth_attempts < self.max_vga_auth_attempts:
                    self.logger.info(
                        f"Caught VGA console password prompt (attempt {self.vga_auth_attempts + 1}/{self.max_vga_auth_attempts})"
                    )
                    self.wait_write(self.password, wait=None)
                    self.vga_auth_attempts += 1
                    if self.vga_auth_attempts < self.max_vga_auth_attempts:
                        self.logger.info(
                            f"Waiting {self.vga_auth_delay} seconds before next VGA auth attempt"
                        )
                        import time

                        time.sleep(self.vga_auth_delay)
                    else:
                        self.logger.info(
                            "Maximum VGA authentication attempts reached. Please use VNC to complete setup."
                        )
                else:
                    self.logger.info(
                        "Max VGA auth attempts reached, letting manual VNC session handle authentication"
...
class XRv9k_Installer(XRv9k):
    ...
    Note: For VGA images (like xrv9k-fullk9-x.vrr.vga-7.1.1.qcow2), the
    install process cannot work automatically because they require
    VGA console access for first boot. The install mode will be
    automatically skipped for VGA images with a warning.
    """
    ...
    def install(self):
        ...
        # Check if this is a VGA image
        if xrv.is_vga_image:
            self.logger.warning("=" * 70)
            self.logger.warning("VGA IMAGE DETECTED: %s", xrv.image)
            self.logger.warning(
                "This image requires VGA console access for first boot."
            )
            self.logger.warning(
                "Install mode (INSTALL=true) cannot work automatically."
            )
            self.logger.warning("Skipping automated installation for VGA image.")
            self.logger.warning("First boot requires VNC access on port 5900.")
            self.logger.warning("Connect via VNC to complete initial setup.")
            self.logger.warning("=" * 70)
            self.logger.info(
                "Automated installation skipped for VGA image. Docker build will continue."
            )
            self.logger.info(
                "VGA image will have VNC enabled for manual first boot setup."
            )
            # Skip automated installation for VGA images
            # VNC is already enabled in __init__ for VGA images
            return
